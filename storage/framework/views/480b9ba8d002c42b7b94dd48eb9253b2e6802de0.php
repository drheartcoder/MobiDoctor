        ...
        <div class="container">
            <div class="row">
                ...
                <div class="my-consultation-wrapper row">
                    ...
                    <?php if( isset( $arr_consultation ) && !empty( $arr_consultation ) ): ?>
                            
                        <?php
                            $id = isset( $arr_consultation['id'] ) && !empty( $arr_consultation['id'] ) ? base64_encode( $arr_consultation['id'] ) : '';
                            $consult_id   = isset( $arr_consultation['consultation_id'] ) && !empty( $arr_consultation['consultation_id'] ) ? base64_encode( $arr_consultation['consultation_id'] ) : '0';
                        ...
                        <div id="video" style="margin:0 auto;">
                            ...
                            <div class="big-vdo-bx" id="agora_remote" style="float:left; width:100% ;height:607px;display:inline-block;"></div>
                                 <div class="sml-vdo-bx" id="agora_local" style="float:right;width:210px;height:147px;display:inline-block;"></div>
                                
                                 <div class="vdo-main-time" id="hms"><?php echo e($doctor_call_duration); ?></div>
...
<!--                                <button id="unmute" class="btn btn-primary" onclick="enableAudio()">Unmute</button>-->
                                <button id="mute" class="btn btn-primary" onclick="disableAudio()"><i class="fa fa-microphone"></i></button>

                                <button id="start_camera" class="btn btn-primary" onclick="enableVideo()"><i class="fa fa-video-camera" ></i></button>
                                 <button id="leave" class="btn btn-primary" onclick="leave()"><i class="fa fa-phone"></i></button>
<!--                                <button id="stop_camera" class="btn btn-primary" onclick="disableVideo()">Stop Camera</button>-->
    ...
    <script src="<?php echo e(url('/')); ?>/public/video/AgoraRTCSDK.js"></script>
    <script src="<?php echo e(url('/')); ?>/public/video/video_jquery.js"></script>

    <script type="text/javascript">
        var current_url = "<?php echo e(url()->previous()); ?>";
        ...
        function join()
        {
            var consult_id = "<?php echo e(base64_decode( isset($consult_id) ? $consult_id : '0' )); ?>";
            var token      = "<?php echo e(csrf_token()); ?>";

            $.ajax({
                url     : "<?php echo e($module_url_path); ?>/upcoming/video/initiate_doctor_call",
                type    : "GET",
                ...
                success : function(res)
                {
                    if(res == 'busy')
                    {
                        swal({
                            title             : "",
                            text              : 'Patient is busy with another Call',
                            type              : 'warning',
                    ...
                    if(res == 'success')
                    {
                        ...
                        var channel = "<?php echo e(base64_decode( isset($consult_id) ? $consult_id : '0' )); ?>";
                        var channel_key = null;

                        //console.log("Init AgoraRTC client with App ID: <?php echo e(env('VIDEO_APP_ID')); ?>");
                        client = AgoraRTC.createClient({mode: 'live'});

                        client.init("<?php echo e(env('VIDEO_APP_ID')); ?>", function () 
                        {
        ...
        function leave()
        {
            ...
            $.ajax({
                url     : "<?php echo e($module_url_path); ?>/upcoming/video/drop_doctor_call",
                type    : "GET",
        ...
        function update_doctor_call_duration()
        {
            var token        = "<?php echo e(csrf_token()); ?>";
            var consult_id   = "<?php echo e(base64_decode( isset($consult_id) ? $consult_id : '0' )); ?>";
            var call_timer   = $('#hms').text();
            ...
            $.ajax({
                url     : "<?php echo e($module_url_path); ?>/upcoming/video/update_doctor_call_duration",
                type    : "GET",
                data    : { _token:token, consult_id:consult_id, call_timer:call_timer },
                success : function(res)
                {
                    if(res.call_status != '')
                    {
                        ...
                        if( res.call_status == 'waiting_for_patient' )
                        {  }

                        if( res.call_status == 'reject_by_patient' )
                        {
                            clearInterval(video_call_timer);

                            swal({
                                title              : 'Call Rejected',
                                text               : 'Patient has rejected the call.',
                                type               : 'warning',
                                showCancelButton   : false,
                                confirmButtonColor : '#63c2de',
                                confirmButtonText  : 'Ok',
