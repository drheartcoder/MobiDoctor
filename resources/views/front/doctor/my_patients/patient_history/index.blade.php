            ...
            <div class="col-sm-8 col-md-9 col-lg-9">
                @include('front.layout._operation_status')
                  ...
                  <div class="pati-new-tab">
                        <ul>
                            <li>
                                <a class="active" id="patient_history" href="{{$module_url_path}}/patient_history/{{$patient_user_id}}">Patient History</a>
                            </li>
                            <li>
                    ...
                    <div class="content">
                            <div class="white-wrapper prescription-wrapper personal-information-new tab-patient">
                                <div class="upcoming-main-details-title-strip">
                                    ...
                                    <div class="upcoming-con-edit">
                                        {{-- href="{{$module_url_path}}/patient_history/download_personal_information/{{$patient_user_id}}" --}}
                                        <a id="btn_download_patient_details" target="_blank" download><i class="fa fa-download"></i></a>
                                        <a href="{{$module_url_path}}/patient_history/edit/{{$patient_user_id}}"><i class="fa fa-pencil-square-o"></i></a>
                                    </div>
                                ...
                                <div class="prescription-section">
                                    <div class="personal-info-bx">
                                        ...
                                        ?>
                                        <img style="width:170px !important;height:154px !important;" src="{{$profile_image}}" alt="info" />
                                    </div>
                                    <div class="patients-new-con-info">
                                        <div class="patients-info-name-bx">
                                            <div class="patient-history-name">Name</div>
                                            <div class="history-name-patient">{{isset($arr_patient_details['first_name'])?decrypt_value($arr_patient_details['first_name']):''}} {{isset($arr_patient_details['last_name'])?decrypt_value($arr_patient_details['last_name']):'-'}}</div>
                                        </div>
                                        <div class="patients-info-name-bx">
                                            <div class="patient-history-name">Date of Birth</div>
                                            <div class="history-name-patient">{{isset($arr_patient_details['date_of_birth'])?date('d M Y',strtotime($arr_patient_details['date_of_birth'])):'-'}}</div>

                                        </div>
                                        <div class="patients-info-name-bx">
                                            <div class="patient-history-name">Gender</div>
                                            <div class="history-name-patient">{{isset($arr_patient_details['gender'])?$arr_patient_details['gender']:''}}</div>
                                        </div>
                                        <div class="patients-info-name-bx">
                                            <div class="patient-history-name">Contact No.</div>
                                            <div class="history-name-patient">{{isset($arr_patient_details['contact_no'])?$arr_patient_details['contact_no']:'-'}}</div>
                                        </div>
                                        <div class="patients-info-name-bx">
                                            <div class="patient-history-name">Mobile No.</div>
                                            <div class="history-name-patient">+{{isset($arr_patient_details['phone_code'])?$arr_patient_details['phone_code']:''}} {{isset($arr_patient_details['mobile_no'])?$arr_patient_details['mobile_no']:'-'}}</div>
                                        </div>
                                        <div class="patients-info-name-bx">
                                            <div class="patient-history-name">Email</div>
                                            <div class="history-name-patient">{{isset($arr_patient_details['email'])?$arr_patient_details['email']:'-'}}</div>
                                        </div>
                                        <div class="patients-info-name-bx">
                                            <div class="patient-history-name">Address</div>
                                            <div class="history-name-patient">{{isset($arr_patient_details['address'])?decrypt_value($arr_patient_details['address']):'-'}}</div>
                            ...
                            <div class="family-member-section">
                                ...
                                <div class="family-add-member"><a href="{{$module_url_path}}/patient_history/family_member/{{base64_encode($arr_patient_details['id'])}}">+ Add Member</a></div>
                                <div class="clearfix"></div>
                                <div class="row">
                                    @if(isset($arr_patient_details['family_member']) && sizeof($arr_patient_details['family_member'])>0)
                                    @foreach($arr_patient_details['family_member'] as $family_key => $family_member)
                                        <div class="col-sm-6 col-md-4 col-lg-4">
                                            <div class="patient-photo-bx">
                                                <div class="patient-image-new"> <img src="{{$default_img_path}}/profile.jpeg" alt="" /> </div>
                                                <div class="patient-status-name">
                                                     @if(isset($arr_patient_details['is_online']) && $arr_patient_details['is_online']!='' && $arr_patient_details['is_online'] == '0')
                                                    ...
                                                    {{isset($family_member['first_name'])?decrypt_value($family_member['first_name']):'-'}} {{isset($family_member['last_name'])?decrypt_value($family_member['last_name']):'-'}}</div>
                                                <div class="patient-birth-date"><i class="fa fa-birthday-cake"></i>{{ isset($family_member['birth_date']) ? date('d M Y',strtotime($family_member['birth_date'])) : '' }}</div>
                                                <div class="dot-menu-section">
                                                    <div class="dot-img-icon">
                                                        <img src="{{url('/')}}/public/front/images/dot-menu.png" alt="" />
                                                        <ul class="hide-dot">
                                                            <li><a onclick="return view_family_member({{$family_member['id']}})">View Member</a></li>
                                                            <li><a onclick="return edit_family_member({{$family_member['id']}})">Edit Member</a></li>
                                                            <li><a onclick="confirm_action(this,event,'Do you really want to delete family member?')" href="{{$module_url_path}}/patient_history/family_member/delete/{{base64_encode($family_member['id'])}}">Delete Member</a></li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
...
<div class="modal fade availability-modal add-member-popup" id="edit-family-member" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <button type="button" class="close close_btn" data-dismiss="modal"><img src="{{url('/')}}/public/front/images/close.png" class="img-responsive" alt="" /></button>
            <div class="modal-body">
                <h4>Edit Member</h4>
                <form name="frm_edit_family_member" id="frm_edit_family_member" method="post" action="{{$module_url_path}}/patient_history/family_member/update">
                    {{csrf_field()}}
...
function view_family_member(id)
{
    var member_id = id;
    $.ajax({
        url         : '{{ $module_url_path }}/patient_history/family_member/deails/'+member_id,
        type        : 'get',
        processData : false,
        contentType : false,
        cache       : false,
...
$("#btn_edit_family_member").click(function()
{
    ...
    else
    {
        ...
        $.ajax({
            url         : '{{ $module_url_path }}/patient_history/family_member/update',
            type        : 'post',
            data        : formData,
            processData : false,
            contentType : false,
...
$("#btn_download_patient_details").click(function(){
     ...
     $.ajax({
               url:"{{$module_url_path}}/patient_history/get_medication_details/{{$patient_user_id}}",
               type:'get',
               success:function(response)
               {
                    $.each(response,function(index,value){
                     ...
                     $.ajax({
                        url:"{{$module_url_path}}/patient_history/download_personal_information/{{$patient_user_id}}",
                        type:'post',
                        //beforeSend : showProcessingOverlay(),
                       ...
                       success:function(response)
                       {
                            ...
                            pdf_url = "{{$module_url_path}}/patient_history/download_personal_information/{{$patient_user_id}}";
