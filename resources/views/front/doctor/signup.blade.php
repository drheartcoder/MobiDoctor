...
<div class="page-wrapper doctor-signup-wrapper">
    <div class="container">
        <div class="doctor-signup-section">
            ...
            <form method="POST" name="doctor_signup_form" id="doctor_signup_form" autocomplete="off" action="{{ url('/') }}/doctor/signup/store">
                {{ csrf_field() }}
                ...
                <div class="doctor-signup-form">
                    ...
                    <div class="form-group country-block">
                        <input type="text" class="daddress" placeholder="Address" name="address" id="autocomplete" value="{{ $browser_address or '' }}" maxlength="300" />
                        <div class="error" id="err_daddress"></div>
                    </div>

                    <div class="form-group" id="dreferral_code_div" ><!--invalid or valid or info-msg-->
                        <input type="text" placeholder="Referral Code" name="referral_code" id="dreferral_code" maxlength="10" />
                        <div class="info-popup">Enter Referral Code</div>
                        <div class="error" id="err_dreferral_code"></div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#dreferral_code').on('blur keyup',function()
        {
            $('#err_dreferral_code').show();
            $('#err_dreferral_code').html('');
            var dreferral_code = $(this).val();
           
            if($.trim(dreferral_code) != '')
            {
                ...
                $.ajax({
                    ...
                    type     : "POST",
                    dataType : 'json',
                    data     : {_token:token, referral_code:dreferral_code},
                    success  : function(res)
                    {
                        if($.trim(res.status) == 'error')
                        {
                            $('#err_dreferral_code').html('Please enter valid referral code.');
                            $('#btn_submit_doctor_signup').attr('disabled',true);

                            $("#dreferral_code_div").addClass("info-msg");
                            $("#dreferral_code_div").removeClass("valid");
                            $("#dreferral_code_div").removeClass("invalid");
                            return false; 
                        }
                        else if($.trim(res.status) == 'success')
                        {
                            ...
                            $("#dreferral_code_div").addClass("valid");
                            $("#dreferral_code_div").removeClass("invalid");
                            $("#dreferral_code_div").removeClass("info-msg");
                            return true;
                        }
                        else
                        {
                            $('#err_dreferral_code').html('Something has get wrong please try again later.');
