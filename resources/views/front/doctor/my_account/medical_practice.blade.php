        ...
        <div class="row">
            ...
            <div class="col-sm-8 col-md-9 col-lg-9">
                ...
                <div class="white-wrapper prescription-wrapper">
                    ...
                    <form name="frm_medical_practice" id="frm_medical_practice" method="post" action="{{ $module_url_path }}/medical_practice/update">
                        {{ csrf_field() }}
                        <div class="prescription-section">
                                        ...
                                        <select name="experience" id="experience">
                                            <option value="">Select Experience</option>
                                                <?php
                                             	for($i=1;$i<100;$i++) {
                                                ?>
                                                <option value="{{ $i }}" @if(isset($arr_doctor_details['experience']) && $arr_doctor_details['experience']!='' && $arr_doctor_details['experience'] == $i) selected @endif>{{$i}} Year(s)</option>
                                            <?php } ?>
                                        </select>
                            ...
                            <div class="row">
                                        ...
                                        <option value="">Code</option>
                                            @if(!empty($mobcode_data) && isset($mobcode_data))
                                                @foreach($mobcode_data as $mobcode)
                                                    <option value="{{ $mobcode['phonecode'] }}" @if(isset($arr_doctor_details['clinic_phone_code']) && $arr_doctor_details['clinic_phone_code'] == $mobcode['phonecode']) selected @endif>+{{ $mobcode['phonecode'] }} ({{ $mobcode['iso3'] }})</option>
                                                @endforeach
                                            @endif
                                        </select>
                                ...
                                <div class="col-sm-12 col-md-4 col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" placeholder="Mobile Number" name="mobile_no" id="mobile_no" maxlength="15"  value="{{isset($arr_doctor['mobile_no'])?$arr_doctor['mobile_no']:''}}"/>
                                        ...
                                        <select id="multi-select-demo" name="language[]" multiple="multiple">
                                            @if(isset($arr_language) && sizeof($arr_language)>0)
                                                @foreach($arr_language as $key => $language)
                                                    <option value="{{$language['id']}}" @if(isset($selected_language) && sizeof($selected_language)>0 && in_array($language['id'],$selected_language)) selected @endif>{{$language['language']}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                            ...
                            <input type="hidden" id="check_experience" name="check_experience" value="{{isset($arr_doctor_details['experience'])?$arr_doctor_details['experience']:''}}">
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        var clinic_name = '{{ isset($arr_doctor_details['clinic_name']) ? $arr_doctor_details['clinic_name'] : '' }}';
        var experience  = '{{ isset($arr_doctor_details['experience']) ? $arr_doctor_details['experience'] : '' }}';
        var address     = '{{ isset($arr_doctor_details['clinic_address']) ? $arr_doctor_details['clinic_address'] : '' }}';
        var contact_no  = '{{ isset($arr_doctor_details['clinic_contact_no']) ? $arr_doctor_details['clinic_contact_no'] : '' }}';
        var mobile_no   = '{{ isset($arr_doctor_details['clinic_mobile_no']) ? $arr_doctor_details['clinic_mobile_no'] : '' }}';
        var email       = '{{ isset($arr_doctor_details['clinic_email']) ? $arr_doctor_details['clinic_email'] : '' }}';
        var phone_code  = '{{ isset($arr_doctor_details['clinic_phone_code']) ? $arr_doctor_details['clinic_phone_code'] : '' }}';
	...
	$("#btn_medical_practice").click(function(){
        ...
        else
        {
            ...
            api.cards.get(card_id).then(function (cards)
            {
                ...
                $.ajax({
                    url  : "{{ $module_url_path }}/medical_practice/update",
                    type : 'post',
                    data : {
                                _token              : _token,
