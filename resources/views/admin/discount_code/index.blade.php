                ...
                <div class="table-responsive" style="border:0">
                  <input type="hidden" name="multi_action" value="" />
                    <table class="table table-advance"  id="table1" >
                        ...
                        <tbody>
                            @if(isset($arr_discount_code) && sizeof($arr_discount_code)>0)
                                @foreach($arr_discount_code as $data)
                                    <tr>
                                        <td><input type="checkbox" name="checked_record[]"  value="{{ base64_encode($data['id']) }}" /></td>
                                        <td>{{isset($data['code'])?$data['code']:''}}</td>
                                        <td>{{isset($data['price'])?decrypt_value($data['price']):''}}</td>
                                        <td>{{isset($data['start_date'])?$data['start_date']:''}}</td>
                                        <td>{{isset($data['end_date'])?$data['end_date']:''}}</td>
                                        <td>
                                            @if($data['status']=="1")
                                        ...
                                        <td>
                                            <a class="btn btn-circle btn-to-success btn-bordered btn-fill show-tooltip" href="{{ $module_url_path.'/edit/'.base64_encode($data['id']) }}"  title="Edit Discount Code">
                                            <i class="fa fa-edit" ></i>
                                            </a>  
                                           {{--  &nbsp;
                                            <a class="btn btn-circle btn-to-success btn-bordered btn-fill show-tooltip" href="{{ $module_url_path.'/view/'.base64_encode($data['id']) }}"  title="Display Discount Code">
                                            <i class="fa fa-eye" ></i>
                                            </a> --}}
                                            &nbsp;
